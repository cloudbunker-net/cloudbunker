...
export const postDeleteFiles = async (req: any, res: Response, next: NextFunction) => {
		...
		if (nameFile.isDir === true) {
			...
			const deleteFolder = async (folderId: any) => {
				...
				await prisma.file.updateMany({
					...
					data: {
						childrenIds: parentFolder.childrenIds.filter((childId: any) => childId !== folder.id),
					},
				});
			};
			...
			await prisma.file
				.updateMany({
					...
					data: {
						childrenIds: parentFolder.childrenIds.filter((childId: any) => childId !== nameFile.id),
					},
				})
				.catch((err) => {
